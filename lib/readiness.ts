...
export interface ReadinessFacts {
  ...
  /** كم مريضًا فعّالًا في السجلّ — مقامُ نسبة من يقدر على البوّابة. */
  patientCount: number;
  /**
   * وكم منهم **بلا جوالٍ مسجَّل**.
   *
   * والدخولُ إلى البوّابة رقمُ الملف **والجوال**، فمريضٌ بلا جوالٍ لا يستطيع
   * الدخول أبدًا مهما أُرسل إليه الرابط — ولا شيء في الشاشة يقول ذلك، فيُظنّ
   * أنّ البوّابة معطوبة.
   */
...
export function readinessChecks(facts: ReadinessFacts): ReadinessCheck[] {
  ...
  /*
   * بوّابة المرضى — بُنيت وتعمل، وكان لا شيء في البرنامج يدلّ عليها.
   *
   * ووجودُها في هذه الشاشة نصفُ الفائدة: هنا يُقرأ ما ينقص قبل التشغيل، وبوّابةٌ
   * لا يعرف المالك أنّها موجودة لا يُرسلها إلى أحد.
   *
   ...
   * يظنّ البوّابة معطوبة وهي تعمل.
   */
  checks.push({
    key: "portal",
    title: "بوّابة المرضى",
    level: facts.patientCount > 0 && facts.patientsWithoutPhone === facts.patientCount
      ? "blocked"
      : facts.patientsWithoutPhone > 0 ? "warn" : "ok",
    detail: facts.patientCount === 0
      ? "لا مرضى بعد"
      : `${facts.patientCount} مريضًا · ${facts.patientsWithoutPhone} بلا جوال`,
    why: facts.patientsWithoutPhone > 0
      ? "الدخول إلى البوّابة رقمُ الملف والجوال المسجَّل. فمن لا جوالَ له لا يدخل مهما أُرسل "
        + "إليه الرابط. سجّل جوالاتهم، ثم أرسل لكلٍّ بوّابته من زرّ واتساب في ملفّه."
      : "البوّابة على /portal، ويدخلها المريض برقم ملفه وجواله. أرسلها له من زرّ واتساب في ملفّه.",
    href: "/patients",
  });
